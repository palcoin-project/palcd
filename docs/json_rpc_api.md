...
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the btcd
  ...
  in the btcd home directory (which is typically `%LOCALAPPDATA%\Palcd` on
  Windows and `~/.palcd` on POSIX-like OSes)
...
|Notes|<font color="orange">Palcd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for btcd.</font>|
...
func main() {
	...
	// have one.
	btcdHomeDir := btcutil.AppDataDir("palcd", false)
