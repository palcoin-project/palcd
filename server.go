...
import (
	...
	"github.com/palcoin-project/palcd/mining/cpuminer"
	"github.com/palcoin-project/palcd/netsync"
	...
	"github.com/palcoin-project/palcutil/bloom"
)
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a palcutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := palcutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a palcutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := palcutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > palcutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, palcutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *palcutil.Tx) {
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *palcutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
