...
const (
	...
	DefaultMinRelayTxFee = palcutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee palcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > palcutil.MaxSatoshi {
		minFee = palcutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *palcutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func IsDust(txOut *wire.TxOut, minRelayTxFee palcutil.Amount) bool {
...
func checkTransactionStandard(tx *palcutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee palcutil.Amount,
...
func GetTxVirtualSize(tx *palcutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
