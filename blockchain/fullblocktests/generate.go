...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: palcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := palcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*palcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, palcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount palcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee palcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee palcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee palcutil.Amount) *wire.MsgTx {
