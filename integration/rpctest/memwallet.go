...
type memWallet struct {
	...
	addrs map[uint32]palcutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]palcutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*palcutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          palcutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (palcutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []palcutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt palcutil.Amount,
	feeRate palcutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := palcutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate palcutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate palcutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt palcutil.Amount
	for _, output := range outputs {
		outputAmt += palcutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() palcutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance palcutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (palcutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := palcutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
