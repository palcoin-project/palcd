...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt palcutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(palcutil.Amount(5 * palcutil.SatoshiPerBitcoin))
	...
	txid = genSpend(palcutil.Amount(500 * palcutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(palcutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*palcutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, palcutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := palcutil.Amount(250 * palcutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = palcutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := palcutil.Amount(50 * palcutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := palcutil.Amount(numMatureOutputs * 50 * palcutil.SatoshiPerBitcoin)
