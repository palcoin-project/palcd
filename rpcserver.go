...
// should ask a connected instance of palcwallet.
...
// These commands will be implemented in palcwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount*palcutil.SatoshiPerBitcoin > palcutil.MaxSatoshi {
			return nil, &btcjson.RPCError{
		...
		addr, err := palcutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &btcjson.RPCError{
		...
		satoshi, err := palcutil.NewAmount(amount)
		if err != nil {
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []btcjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = palcutil.Amount(v.Value).ToBTC()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	txReply := &btcjson.TxRawResult{
		...
		Vsize:    int32(mempool.GetTxVirtualSize(palcutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(palcutil.NewTx(mtx))),
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := palcutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := palcutil.NewBlockFromBytes(blkBytes)
	if err != nil {
...
func handleGetBlockTemplateRequest(s *rpcServer, request *btcjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &btcjson.RPCError{
			...
			Message: "Palcoin is not connected",
		}
	}
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &btcjson.RPCError{
			...
			Message: "Palcoin is downloading blocks...",
		}
	}
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &btcjson.GetTxOutResult{
		...
		Value:         palcutil.Amount(value).ToBTC(),
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &btcjson.PrevOut{
				...
				Value:     palcutil.Amount(originTxOut.Value).ToBTC(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr palcutil.Address, numToSkip, numRequested uint32) ([]*palcutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	params := s.cfg.ChainParams
	addr, err := palcutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
...
const messageSignatureHeader = "Palcoin Signed Message:\n"
...
func handleSignMessageWithPrivKey(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	wif, err := palcutil.DecodeWIF(c.PrivKey)
	if err != nil {
		message := "Invalid private key"
		switch err {
		case palcutil.ErrMalformedPrivateKey:
			message = "Malformed private key"
		case palcutil.ErrChecksumMismatch:
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	addr, err := palcutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if _, ok := addr.(*palcutil.AddressPubKeyHash); !ok {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCType,
	...
	address, err := palcutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err == nil {
			...
			//
			// Palcd does not respond to any request without and "id" or "id":null,
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *palcutil.Block, flags blockchain.BehaviorFlags) (bool, error)
