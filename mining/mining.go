...
const (
	...
	MinHighPriority = palcutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr palcutil.Address) (*palcutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *palcutil.Tx, height int32) error {
...
func logSkippedDeps(tx *palcutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress palcutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*palcutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := palcutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := palcutil.NewBlock(&msgBlock)
...
func AddWitnessCommitment(coinbaseTx *palcutil.Tx,
	blockTxns []*palcutil.Tx) []byte {
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A palcutil.Block should use saved in the state to avoid
