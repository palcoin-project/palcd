...
type GetTxOutSetInfoResult struct {
	...
	TotalAmount    palcutil.Amount `json:"total_amount"`
}
...
func (g *GetTxOutSetInfoResult) UnmarshalJSON(data []byte) error {
	...
	amount, err := palcutil.NewAmount(aux.TotalAmount)
	if err != nil {
		return err
	}
...
// field since it requires wallet access, which is outside the scope of palcd.
...
func (f *FundRawTransactionResult) UnmarshalJSON(data []byte) error {
	...
	fee, err := palcutil.NewAmount(rawRes.Fee)
	if err != nil {
		return err
	}
