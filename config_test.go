...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-palcd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "palcd")
	if err != nil {
	...
	tmpConfigFile := filepath.Join(appPath, "sample-palcd.conf")
