...
var (
	btcdHomeDir           = palcutil.AppDataDir("palcd", false)
	btcctlHomeDir         = palcutil.AppDataDir("palcctl", false)
	btcwalletHomeDir      = palcutil.AppDataDir("palcwallet", false)
	defaultConfigFile     = filepath.Join(btcctlHomeDir, "palcctl.conf")
...
// config defines the configuration options for palcctl.
//
...
func normalizeAddress(addr string, chain *chaincfg.Params, useWallet bool) (string, error) {
	...
	if err != nil {
		...
		case &chaincfg.TestNet3Params:
			if useWallet {
				defaultPort = "11987" //18332"
			} else {
				defaultPort = "11967" //18334
			}
		case &chaincfg.SimNetParams:
			if useWallet {
				defaultPort = "11916" //18554"
			} else {
				defaultPort = "11918" //18556"
			}
		case &chaincfg.RegressionNetParams:
			if useWallet {
				// TODO: add port once regtest is supported in palcwallet
				paramErr := fmt.Errorf("cannot use -wallet with -regtest, palcwallet not yet compatible with regtest")
				return "", paramErr
			} else {
				defaultPort = "11967" //18334"
			}
		case &chaincfg.SigNetParams:
			if useWallet {
				defaultPort = "31987" //38332"
			} else {
				defaultPort = "31987" //"38332"
			}
		default:
			if useWallet {
				defaultPort = "1987" //8332"
			} else {
				defaultPort = "1967" //8334"
			}
		}
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default palcctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(btcwalletHomeDir, "palcwallet.conf")
		} else {
			serverConfigPath = filepath.Join(btcdHomeDir, "palcd.conf")
		}
...
// palcwallet), and extract the RPC user and password from it.
