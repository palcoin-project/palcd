...
// newAddressPubKey returns a new palcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) palcutil.Address {
	addr, err := palcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new palcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) palcutil.Address {
	addr, err := palcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new palcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) palcutil.Address {
	addr, err := palcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the palcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the palcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := palcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := palcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := palcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := palcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := palcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*palcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*palcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*palcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*palcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*palcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
